// ===== 24. SIGN FIND - 부호 찾기 =====
let sfScore,sfTime,sfTotal,sfQTimer,sfQTime,sfQLimit,sfCombo;
function initSignfind(){sfScore=0;sfTime=30;sfTotal=0;sfCombo=0;document.getElementById('sf-score').textContent='0점';initHearts('sf');
...
document.getElementById('sf-eq').innerHTML=a+' <span class="sf-blank">?</span> '+b+' = '+r;
...
function sfPick(el,picked,answer){if(sfQTime<=0)return;if(el.classList.contains('ok')||el.classList.contains('no'))return;clearInterval(sfQTimer);freezeQBar('sf-qbar');sfTotal++;
if(picked===answer){el.classList.add('ok');sfCombo++;const pct=sfQTime/sfQLimit;const bonus=pct>.75?5:pct>.5?3:1;sfScore+=10+bonus;setScore('sf-score',sfScore);if(sfCombo%5===0){sfTime=Math.min(sfTime+COMBO_TIME_BONUS,99);toast(sfCombo+'콤보! +'+COMBO_TIME_BONUS+'초')}}
else{el.classList.add('no');sfCombo=0;document.querySelectorAll('.sf-opt').forEach(o=>{if(o.textContent===answer)o.classList.add('ok')});
