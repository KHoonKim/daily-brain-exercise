...
else if(k==='OK'){if(window._flInput.length<window._flLen){return}if(window._flInput===flAnswer){flScore+=10+flLv*5;flLv++;
setScore('fl-score',flScore);document.getElementById('fl-level').textContent='Lv.'+flLv;toast('정답!')}
else{document.getElementById('fl-display').textContent=flAnswer;document.getElementById('fl-display').style.color='var(--no)';curScore=flScore;setHeartResumeCallback(()=>{document.getElementById('fl-display').style.color='';flRound()});if(loseHeart('fl')){return}setTimeout(()=>{document.getElementById('fl-display').style.color='';flRound()},800);return}
