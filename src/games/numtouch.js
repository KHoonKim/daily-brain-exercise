...
function ntTap(el,n){if(n!==ntNext){curScore=0;setHeartResumeCallback(()=>{});if(loseHeart('nt')){clearInterval(ntTimer);return;}return;}
if(!ntStart){ntStart=Date.now();ntTimer=setInterval(()=>{document.getElementById('nt-timer').textContent=((Date.now()-ntStart)/1000).toFixed(1)+'s'},100)}
