...
function initMath(){
  ...
  setTickFn(mathTick);curTimer=setInterval(mathTick,1000);
}
function mathTick(){mathTime--;mathUpdateT();if(mathTime<=0){clearInterval(curTimer);setTimeExtendResumeCallback((s)=>{mathTime=s;mathUpdateT();curTimer=setInterval(mathTick,1000);mathGen()});showResult(mathScore,'암산 챌린지',[{val:mathTotal,label:'문제 수'},{val:mathMaxCombo+'x',label:'최대 콤보'}],{combo:mathMaxCombo, _isTimerEnd:true})}}
