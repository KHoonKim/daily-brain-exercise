...
function initFocus(){focusScore=0;focusTime=30;focusHit=0;focusMiss=0;document.getElementById('focus-score').textContent='0점';document.getElementById('focus-timer').textContent='30s';document.getElementById('focus-timer').className='g-timer';document.getElementById('focus-field').innerHTML='';clearInterval(curTimer);clearInterval(focusSpawnTimer);curTimer=setInterval(focusTick,1000);focusSpawnTimer=setInterval(spawnTarget,800);spawnTarget();spawnTarget()}
function focusTick(){focusTime--;document.getElementById('focus-timer').textContent=focusTime+'s';if(focusTime<=10)document.getElementById('focus-timer').className='g-timer urgent';if(focusTime<=0){clearInterval(curTimer);clearInterval(focusSpawnTimer);setTimeExtendResumeCallback((s)=>{focusTime=s;document.getElementById('focus-timer').textContent=focusTime+'s';document.getElementById('focus-timer').className='g-timer';curTimer=setInterval(focusTick,1000);clearInterval(focusSpawnTimer);focusSpawnTimer=setInterval(spawnTarget,800);spawnTarget()});showResult(focusScore,'집중력 탭',[{val:focusHit,label:'명중'},{val:focusMiss,label:'실수'}], {_isTimerEnd:true})}}
