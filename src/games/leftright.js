// ===== 34. LEFT RIGHT - 좌우 판단 =====
let lrScore,lrTime,lrAns,lrTotal=0,lrQTimer,lrQTime,lrQLimit,lrLv,lrCombo;
function initLeftright(){lrScore=0;lrTime=30;lrLv=0;lrCombo=0;document.getElementById('lr-score').textContent='0점';initHearts('lr');
...
document.getElementById('lr-msg').textContent='어느 손인지 맞춰보세요.';
...
function lrPick(d){if(lrQTime<=0)return;if(d===lrAns){clearInterval(lrQTimer);freezeQBar('lr-qbar');lrLv++;lrCombo++;const pts=lrCombo>=5?20:10;lrScore+=pts;setScore('lr-score',lrScore);if(lrCombo%5===0){lrTime=Math.min(lrTime+COMBO_TIME_BONUS,99);toast(lrCombo+'콤보! +'+COMBO_TIME_BONUS+'초')}else{toast('정답!')}}
else{clearInterval(lrQTimer);lrCombo=0;setHeartResumeCallback(lrGen);curScore=lrScore;if(loseHeart('lr'))return}
