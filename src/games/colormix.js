...
const CMIX=[
  ...
  {a:{name:'파랑',hex:'#3182F6'},b:{name:'초록',hex:'#1FC58E'},result:{name:'청록',hex:'#0D9488'},wrong:[{name:'보라',hex:'#8B5CF6'},{name:'연두',hex:'#A3E635'},{name:'남색',hex:'#1E3A8A'}]},
  ...
  {a:{name:'빨강',hex:'#F04452'},b:{name:'주황',hex:'#F97316'},result:{name:'다홍',hex:'#FB923C'},wrong:[{name:'초록',hex:'#1FC58E'},{name:'분홍',hex:'#FB7185'},{name:'갈색',hex:'#92400E'}]},
...
function cmxPick(el,picked,answer){if(cmxQTime<=0)return;if(el.classList.contains('ok')||el.classList.contains('no'))return;clearInterval(cmxQTimer);freezeQBar('cmx-qbar');
