...
const oddbar=document.getElementById('odd-qbar');if(oddbar){oddbar.style.transition='none';oddbar.style.width='100%';oddbar.offsetWidth;oddbar.style.transition=`width ${oddQLimit}s linear`;oddbar.style.width='0%'}const oddqt=document.getElementById('odd-q-time');if(oddqt)oddqt.textContent=oddQLimit.toFixed(1)+'s';
oddQTimer=setInterval(()=>{oddQTime-=0.1;document.getElementById('odd-q-time').textContent=Math.max(0,oddQTime).toFixed(1)+'s';if(oddQTime<=0){clearInterval(oddQTimer);curScore=oddScore;setHeartResumeCallback(oddGen);if(loseHeart('odd'))return;scheduleNextQuestion(oddGen,300)}},100)}
function oddPick(el,i,ans){if(oddQTime<=0)return;if(i===ans){clearInterval(oddQTimer);freezeQBar('odd-qbar');el.classList.add('ok');const pct=oddQTime/oddQLimit;const bonus=pct>.75?5:pct>.5?3:1;oddScore+=10+oddLv*2+bonus;oddLv++;setScore('odd-score',oddScore);scheduleNextQuestion(oddGen,400)}else{el.classList.add('no');oddScore=Math.max(0,oddScore-5);setScore('odd-score',oddScore);clearInterval(oddQTimer);freezeQBar('odd-qbar');curScore=oddScore;setHeartResumeCallback(oddGen);if(loseHeart('odd'))return;scheduleNextQuestion(oddGen,400)}}
